...
router.get('/', (req, res) => {
  try {
    const trucks = db.prepare(`
      ...
      ORDER BY t.owned DESC, t.rented DESC, t.price ASC
    `).all();
    res.json(trucks);
  } catch (error) {
...
// Get owned trucks (+ rented) — utilisé pour l'assignation chauffeurs
// FIX: inclut les camions loués car ils peuvent aussi être assignés
router.get('/owned', (req, res) => {
  try {
    const trucks = db.prepare(`
      ...
      FROM trucks t
      LEFT JOIN drivers d ON t.assigned_driver_id = d.id
      WHERE t.owned = 1 OR t.rented = 1
      ORDER BY t.owned DESC, t.name
    `).all();
    res.json(trucks);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.post('/:id/buy', (req, res) => {
  try {
    ...
    // Check garage capacity (seuls les camions owned comptent)
    ...
    // Si le camion était loué, on passe directement en owned (rachat)
    db.prepare('UPDATE trucks SET owned = 1, rented = 0 WHERE id = ?').run(id);
...
// FIX: retourner un camion loué (endpoint manquant — sans ce route, la location était permanente)
router.post('/:id/return', (req, res) => {
  try {
    ...
    if (!truck.rented) {
      return res.status(400).json({ error: 'Ce camion n\'est pas loué' });
    }

    // Désassigner le chauffeur et rendre le camion
    db.prepare('UPDATE trucks SET rented = 0, assigned_driver_id = NULL WHERE id = ?').run(id);

    res.json({ success: true, message: 'Camion rendu avec succès' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/:id/sell', (req, res) => {
  try {
    ...
    // Unassign driver and release truck
